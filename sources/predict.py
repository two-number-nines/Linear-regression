...
def predict_price(mileage: int):
    ...
    if p.exists():
        ...
        if prompt == 'y':
            print("You can decide if you want to see some plots, please pick a number:")
            prompt = input("Do you want to plot after training? y/n\n")
            print('---------------------------------------------------------------------')
            initialize_model(plot=prompt)
            read_and_print(mileage)
        else:
